...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomAdc_ServiceHdle;                    /**< ADC_Service handle */
  uint16_t  CustomAdc_Value_CharacteristicHdle;                  /**< ADC_Value_Characteristic handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeAdc_Value_Characteristic = 2;
...
#define COPY_ADC_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_ADC_VALUE_CHARACTERISTIC_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomAdc_Value_CharacteristicHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_ADC_VALUE_CHARACTERISTIC_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_ADC_VALUE_CHARACTERISTIC_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for ADC_Service +
   *                                2 for ADC_Value_Characteristic +
   *                                1 for ADC_Value_Characteristic configuration descriptor +
   *                              = 4
   *
  ...
  COPY_ADC_SERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomAdc_ServiceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: ADC_Service, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: ADC_Service , handle = 0x%04x \n\r", CustomContext.CustomAdc_ServiceHdle);
  }

  /**
   *  ADC_Value_Characteristic
   */
  COPY_ADC_VALUE_CHARACTERISTIC_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomAdc_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeAdc_Value_Characteristic,
                          CHAR_PROP_READ | CHAR_PROP_NOTIFY,
                          ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_ATTRIBUTE_WRITE,
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomAdc_Value_CharacteristicHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : ADC_VALUE_CHARACTERISTIC, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : ADC_VALUE_CHARACTERISTIC , handle = 0x%04x \n\r", CustomContext.CustomAdc_Value_CharacteristicHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ADC_VALUE_CHARACTERISTIC:
      ret = aci_gatt_update_char_value(CustomContext.CustomAdc_ServiceHdle,
                                       CustomContext.CustomAdc_Value_CharacteristicHdle,
                                       0, /* charValOffset */
                                       SizeAdc_Value_Characteristic, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value ADC_VALUE_CHARACTERISTIC command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value ADC_VALUE_CHARACTERISTIC command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ADC_VALUE_CHARACTERISTIC:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomAdc_ServiceHdle, CustomContext.CustomAdc_Value_CharacteristicHdle, SizeAdc_Value_Characteristic, pPayload);
